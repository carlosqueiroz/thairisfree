...
$userid = $_GET['id'];
$result = mysql_query("SELECT * FROM xray_user WHERE ID='$userid'");

while($row = mysql_fetch_array($result)) //ID CODE DF_CODE NAME LASTNAME  USER_TYPE_CODE  PREFIX CENTER_CODE  ALL_CENTER PACS_LOGIN
	{
			$ID = $row['ID'];
			$CODE =$row['CODE'];
			$DF_CODE = $row['DF_CODE'];
...
echo "<td>";
$result = mysql_query("SELECT TYPE FROM xray_user_type");
...
echo "<tr><td>CENTER CODE</td><td><input type=\"text\" name=\"CENTER_CODE\"  size=50 value=$CENTER_CODE>".$TIME_USE."</td></tr>";
